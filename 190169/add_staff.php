...
// --- ส่วนที่เพิ่ม: คำนวณรหัส B00001 อัตโนมัติ ---
// ดึงตัวเลขที่มากที่สุดจากฐานข้อมูล โดยตัดตัว B (ตัวที่ 1) ออกแล้วแปลงเป็นตัวเลข
$sql_max = "SELECT MAX(CAST(SUBSTRING(staff_id, 2) AS UNSIGNED)) as max_id 
            FROM staff_main 
            WHERE staff_id LIKE 'B%'";
$res_max = mysqli_query($conn, $sql_max);
$row_max = mysqli_fetch_assoc($res_max);

// ถ้ามีข้อมูลอยู่แล้วให้ +1 ถ้ายังไม่มี (เป็นค่าว่าง) ให้เริ่มที่ 1
$next_num = ($row_max['max_id'] > 0) ? $row_max['max_id'] + 1 : 1;

// จัดรูปแบบ: ใส่ 'B' นำหน้า และตามด้วยตัวเลขที่รันไว้ 5 หลัก (เช่น B00001)
...
$row = [
    'cid' => '', 
    'staff_id' => $next_staff_id, // ใส่ค่าที่คำนวณได้ลงไปตรงนี้
    'license_no' => '', 'fname' => '', 'lname' => '',
        ...
        <div class="card-body p-4">
            <form action="save_staff.php" method="POST">
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-4">
                        <label>รหัสเจ้าหน้าที่ (รันอัตโนมัติ)</label>
                        <input type="text" name="staff_id" class="form-control bg-light" 
                            value="<?php echo $next_staff_id; ?>" readonly required>
                    </div>
                    <div class="col-md-2">
                        <label>เพศ</label>
