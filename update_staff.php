<?php
// 1. เพิ่มบรรทัด Debug (เปิดเฉพาะตอนแก้ปัญหา ถ้าแก้เสร็จแล้วให้ลบออกหรือคอมเมนต์ไว้)
...
// 2. ฟังก์ชันเขียน Log (กรณีไม่ได้ Include มาจากที่อื่น)
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $s_group_id = mysqli_real_escape_string($conn, $_POST['s_group_id']);
    $s_dept_id = mysqli_real_escape_string($conn, $_POST['s_dept_id']);
    ...
    // 3. เปรียบเทียบข้อมูลเพื่อทำ Log
    $diff = [];
    $check_fields = [
        'cid' => 'เลขบัตรประชาชน', 'staff_id' => 'รหัสเจ้าหน้าที่', 'license_no' => 'เลขใบประกอบ',
        'position_no' => 'เลขที่ตำแหน่ง', 'fname' => 'ชื่อ', 'lname' => 'นามสกุล',
        'ed_id' => 'การศึกษาสูงสุด', 'prefix_id' => 'คำนำหน้า', 'prefix_academic_id' => 'คำนำหน้าวิชาการ',
        'gender_id' => 'เพศ', 'type_id' => 'ประเภทพนักงาน', 'position_id' => 'ตำแหน่ง',
        'provider_pos_id' => 'ตำแหน่งสายงาน', 'group_id' => 'กลุ่มภารกิจ', 'dept_id' => 'แผนก/ฝ่าย',
        's_group_id' => 'กลุ่มงานจริง', 's_dept_id' => 'หน่วยงานจริง', 'workplace_s' => 'สถานที่จริง',
        'birthday' => 'วันเกิด', 'start_date' => 'วันเริ่มงาน', 'work_status' => 'สถานะการทำงาน'
    ];
    ...
    $sql = "UPDATE staff_main SET 
                ...
                position_no = '$position_no', fname = '$fname', lname = '$lname', 
                ed_id = $ed_sql, gender_id = $gender_sql, type_id = $type_sql, 
                birthday = $bday_sql, start_date = $start_sql, end_date = $end_sql, 
                leave_reason_id = $leave_reason_sql, prefix_id = '$prefix_id', 
                prefix_academic_id = $academic_sql, position_id = '$position_id', 
                provider_pos_id = '$provider_pos_id', group_id = '$group_id', 
                dept_id = '$dept_id', s_group_id = $s_group_sql, 
                s_dept_id = $s_dept_sql, workplace_s = '$workplace_s', 
                work_status = '$work_status', provider_status = '$provider_status', 
                ekyc_status = '$ekyc_status', updated_at = '$updated_at' 
                WHERE cid = '$old_cid'";

    if (mysqli_query($conn, $sql)) {
        write_log($conn, 'EDIT', 'staff_main', $staff_id, $log_details);
        // แสดง SweetAlert พร้อมโครงสร้าง HTML ที่สมบูรณ์
        echo "<!DOCTYPE html>
        <html>
        <head>
